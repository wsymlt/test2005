...
//4.2 书写一个打包html的方法
const htmlHandler = ()=>{
    return gulp.src(['./pages/*.html','./pages/*.htm'])   //找到src目录里面下的pages目录下的所有后缀为.html的文件
    .pipe(htmlmin({
...
const jsonHandler = ()=>{
    return gulp.src('./json/**')  //images文件夹下的所有文档
    .pipe(gulp.dest('./dist/json'))   //放到指定的目录就可以了
}

//6.1 书写一个移动lib文件夹的方法
 const libHandler = ()=>{
   return gulp.src('./sass/**')  //lib文件夹下的所有文档
...
const watchHandler = ()=>{
    //监控着src下的css下的所有css只要你按,只要一发生变化,就会自动执行一遍cssHandler这个任务
    gulp.watch('./css/*.css',cssHandler);
    gulp.watch('./js/*.js',jsHanlder);
...
//         port:'8081', //端口号,0-65535,尽量不使用0-1023
...
module.exports.default = gulp.series(
    delHandler,
    gulp.parallel(cssHandler,jsHanlder,htmlHandler,imgHandler,libHandler,jsonHandler),
    serverHandler,
    watchHandler
)
